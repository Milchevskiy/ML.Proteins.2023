#!/usr/bin/python3
#
# script to learn multi-output regression neural network to predict protein secondary structure
#
...
# If videocard has < 16G DRAM, disable test dataset for learning by test_enabled=0
test_enabled = 1
...
class MultiOutputRegression(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # 3 hidden layer
        self.hidden3 = Linear(80, 50)
        kaiming_uniform_(self.hidden3.weight, nonlinearity='sigmoid')
        self.act3 = torch.nn.Sigmoid()

        # 4 hidden layer and output
        self.hidden4 = Linear(50, 8)
...
BestModelName 		= 'Q8_DATA_MODEL'
BestModelName_CPU 	= 'Q8_DATA_MODEL_500_FOR_CPU'
...
X = numpy.load("train_Q8X.npy")
y = numpy.load("train_Q8Y.npy")
...
for epoch in range(500):
    ...
    for i in range(1, 500):
        ...
        if kk%1000==1:
            ...
            print('q8 for CB513 ', q16)

            q16l=accuracy(yt, y_pred)
            print('q8 for learning sample ', q16l)

            torch.save(model,BestModelName)
            torch.save(model.state_dict(), BestModelName_CPU)

#            if (q16l > 0.8 and q16 > 0.7910):
            if (q16 > 0.7935):
                printf ('Accuracy above 80%% is achieved, final step = %.7f !', lr_start)
                learned = 1
                break

y_pred = model(Xt)
q16=accuracy(yt, y_pred)
printf("Final q8 for learning sample %lf\n",q16)
y_pred_test_1 = model(Xtest_1)
q16=accuracy(ytest_1, y_pred_test_1)
